-- ═══════════════════════════════════════════════════════════════════════
-- 🔥 SERVER UPTIME DETECTOR - USING DistributedGameTime
-- Dùng workspace.DistributedGameTime (chính xác 100%)
-- Theo phân tích: GetServerTimeNow() = Unix timestamp, DistributedGameTime = uptime
...
print("═══════════════════════════════════════════════════════════════")
print("🔥 SERVER UPTIME DETECTOR (DistributedGameTime) - STARTING...")
print("═══════════════════════════════════════════════════════════════")

-- ═════════════════════════════════════════════════════════════════════
-- PHẦN 1: SERVER - Tính Server Start Time (ước tính)
-- ═════════════════════════════════════════════════════════════════════
-- Theo phân tích ChatGPT:
-- serverStartTime = GetServerTimeNow() - DistributedGameTime
...
local function setupPlayerGUI(player)
    ...
    local oldGUI = playerGui:FindFirstChild("ServerUptimePanel")
    if oldGUI then
        oldGUI:Destroy()
    end
...
print("📋 Setting up GUI for existing players...")
...
task.spawn(function()
    while task.wait(60) do
        ...
        print(string.format("[SERVER] Uptime: %02d:%02d:%02d | Players: %d | Method: DistributedGameTime", 
            hours, mins, secs, #Players:GetPlayers()))
    end
end)

print("═══════════════════════════════════════════════════════════════")
print("✅ SERVER UPTIME DETECTOR - READY")
print("💡 Using workspace.DistributedGameTime (100% accurate)")
print("💡 Formula: ServerStartTime = GetServerTimeNow() - DistributedGameTime")
print("═══════════════════════════════════════════════════════════════")
