...
do
    ...
    def("Collect Verify Time", 2)               -- doi bao lau de xac nhan da an chest (giay)
...
-- NHAY CHEST SIEU NHANH (logic tu SG.txt):
-- SetPrimaryPartCFrame len chest + ep ChangeState(Jumping) moi frame -> touch dang nhay.
-- Them firetouchinterest cho chac, verify bang loot van ra / chest bien mat.
...
local function _teleChest(chest, stopCondition)
    ...
    local verifyTime = _cfg("Collect Verify Time", 2)
    ...
    repeat
        ...
        -- nhay siêu nhanh: teleport len chest + ep jump moi frame
        pcall(function()
        ...
        -- backup: firetouchinterest (touch chac chan hon vat ly)
        pcall(function()
